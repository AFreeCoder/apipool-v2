...
async function setupDb() {
  ...
  await db().insert(schema.walletAccount).values({
...
function requestRow(input: {
  ...
  return {
    id: input.id,
    newapiRequestId:
      input.status === 'settled' ? `newapi-${input.id}` : null,
    userId: 'usage-view-user',
...
test('usage 聚合：spend 只含 settled，请求数含全部状态并按模型分组', async () => {
  ...
  assert.equal(view.summary.inputTokens, 175);
  assert.equal(view.summary.outputTokens, 40);
  assert.equal(view.summary.spendUsd, 1);
  assert.deepEqual(view.summary.byModel, [
    { modelId: 'model-a', requestCount: 2, spendUsd: 1 },
    { modelId: 'model-b', requestCount: 1, spendUsd: 0 },
  ]);
});
...
test('logs 状态映射：open/pending 为 billing，失败免单，settled 带金额', async () => {
  ...
  const byId = new Map<string, any>(
    view.logs.map((row: any) => [row.id, row])
  );
...
test('range 过滤：7d 排除窗口外请求，30d 包含', async () => {
  ...
  assert.equal(seven.logs.some((row: any) => row.id === 'usage-old-b'), false);
  assert.equal(thirty.logs.some((row: any) => row.id === 'usage-old-b'), true);
