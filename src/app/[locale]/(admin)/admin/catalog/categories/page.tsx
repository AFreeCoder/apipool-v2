...
export default async function AdminCatalogCategoriesPage({
  ...
  const categories = (await getCategories()).map((category) => ({
    ...category,
    status: t(`statusOptions.${category.status}`),
  }));

  const crumbs: Crumb[] = [
    { title: t('crumbs.admin'), url: '/admin' },
    { title: t('crumbs.catalog'), is_active: true },
    { title: t('categories.list.crumb'), is_active: true },
  ];
  ...
  return (
    <>
      ...
      <Main>
        <MainHeader title={t('categories.list.title')} />
        <TableCard
          table={table}
          buttons={[
            {
              title: t('categories.list.buttons.new'),
              icon: 'Plus',
              url: '/admin/catalog/categories/new',
            },
          ]}
        />
      </Main>
