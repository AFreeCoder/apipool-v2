...
- USD Checkout 不在代码中固定支付方式，而是由 Stripe Dashboard 根据币种、地区和资格
  动态展示；管理后台的 `stripe_payment_methods` 仅保留给一次性 CNY Checkout。
- Stripe 自动发送成功付款收据和退款邮件；一次性充值不启用付款后发票，避免在普通收据
  之外产生额外的付款后发票费用。
