...
- **[OQ-3 / 来自 Step17 F1，follow-up] roles/users 既有写 handler 同样应二次鉴权**：roles/[id]/edit、edit-permissions、users/[id]/edit、edit-roles 这 4 个**既有**（非本次引入）写 handler 仅 passby null-check、无 requirePermission 二次校验，有同类 Server Action 绕过风险（页首仍有 ROLES_WRITE/USERS_WRITE 门控）。DESIGN §8.1 只要求 catalog/settings，故未纳入本轮；建议另开 follow-up 给这 4 个补 ROLES_WRITE/USERS_WRITE 二次校验，使 admin 写面闭环。
