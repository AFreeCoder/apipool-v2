...
APIPool v2 不需要一次性复制全部体验。第一阶段只保留展示、文档入口、价格和 API Key 管理前端骨架；实际模型调用交给独立 New API 后台，后续再补完整接入文档、支付、用量统计、Playground、活动、内容矩阵和更完整的数据分析。
...
第一阶段文档目标是保留信息架构入口，不追求填充完整内容，也不承诺真实调用已经可用。
