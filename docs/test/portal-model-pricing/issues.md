...
- [ ] New API 成本参照同步后，Embedding 的门户卖价数据未被覆盖，但公开目录把输入价隐藏为 `—`。真实同步把 `source_supported_endpoint_types` 更新为 `["openai"]`，公开查询不再识别其为 Embedding-only，空输出价因而被当作价格配置不完整。最终处理采用现有计价模型：仅对确认无输出 token meter 的 Embedding 模型，把门户输出价明确配置为 `0`；普通 token 模型不得据此配置 0 价。本阶段不额外增加 Embedding 分类特判或新的计价公式。后续复测必须依次验证“输出价设为 0 → 执行成本参照同步 → 公开目录保留输入价并显示 `$0` 输出价 → 发布门禁通过 → 真实 Embedding 调用仍只按输入 token 结算”，通过后方可关闭。证据见 [`live-uat-report.md`](live-uat-report.md)。
...
- [x] Checkout 关闭时，API Key 页改为环境无关的余额说明，不再提示用户前往余额页充值；浏览器复测通过。证据见 [`retest-39b9507.md`](retest-39b9507.md)。
- [x] 同一会话在后台保存 listing 折扣后，通过“首页 → 模型与价格”的客户端路由进入公开目录会立即显示新价格，无需完整刷新或查询参数绕缓存；9.9 折与恢复 10 折双向复测通过。证据见 [`retest-39b9507.md`](retest-39b9507.md)。
- [x] Key 页的已知分组按 slug 本地化，选择器显示“反代”和“官方”，不再向中文用户显示 `Official`；浏览器复测通过。证据见 [`retest-39b9507.md`](retest-39b9507.md)。
