...
- [x] P1-8 新增分组折扣撞唯一索引报原始 SQLite 错误 —— 已修复（`29f29a7`）：捕获 uniq_listing_model_group，抛已翻译提示。**2026-07-09 补注**：抛错路径在生产会被 Next 脱敏，该修复此前实际无效；`2193a2d`（admin-review R-2）改为 `return {status:'error'}` 后翻译提示才真正可见，并有守卫测试防回退。
