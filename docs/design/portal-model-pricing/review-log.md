...
## 第 5 轮（2026-07-20，用户基于第 4 轮实测的收敛裁决）

| 项 | 用户主张 | 评估与裁决 | 落点 |
|---|---|---|---|
| O13 token 计费统一原则 | 对 token 制模型统一采用官方计费方式；usage 没返回的字段按 0 套标准公式——订阅池场景与上游口径一致、官方场景全项计入，任何渠道都不出错，渠道差异最终只是价格系数 | **成立且优于第 4 轮落地**（配置随官方事实而非渠道能力，换渠道零改动、自动生效）。两处精确化：① OpenAI 子集语义下缺字段=分类粗化（总量守恒、按 input 价收），Anthropic 互斥语义下缺字段=整段漏计（直接免费）——两者都只会少收不会多收（O10 精神），量级不同已注明；② "渠道差异只是系数"精确为：卖价结构全渠道统一（官方价 × 折扣），渠道差异只进成本侧不进卖价链。GPT-5.6 cache_write 从"留空"改回**官方满配 6.25** | §7.4 新增统一原则段、§5.3、§10、§12 |
| images 设计覆盖确认 | 图片"要么按次要么按 token"，现有设计是否满足？ | 满足：`billing_scheme` 二选一承载两种形态，meter（image_*）与 tier 表都已就位，切换 = 改字段发新版本、历史账本不受影响。无新增设计项；遗留为执行清单（见 §13） | §13 |
